// js-files/analytics.js

// ===== КОНФИГ =====

const ANALYTICS_URL = (typeof BACKEND_BASE_URL === 'string'
    ? BACKEND_BASE_URL
  ...
  const ANALYTICS_SESSION_TTL_MS = 60 * 60 * 1000;
  ...
  function analyticsSaveSessionId(id) {
    try {
      ...
      localStorage.setItem(ANALYTICS_SESSION_ID_KEY, JSON.stringify(obj));
    } catch (_) {}
  }
  ...
  function analyticsGetBaseContext() {
    ...
    if (tgLocal && tgLocal.initData) {
      ...
      source = 'telegram';
      const u = tgLocal.initDataUnsafe && tgLocal.initDataUnsafe.user;
      if (u) {
        tgUserId = u.id ?? null;
    ...
    const screenW = typeof screen !== 'undefined' ? (screen.width || null) : null;
    const screenH = typeof screen !== 'undefined' ? (screen.height || null) : null;
  ...
  function analyticsScheduleFlush() {
    ...
    analyticsFlushTimer = setTimeout(analyticsFlush, ANALYTICS_FLUSH_INTERVAL_MS);
  }
  ...
  async function analyticsFlush() {
    ...
    const batch = analyticsQueue.slice(0, ANALYTICS_BATCH_SIZE);
    ...
    try {
      ...
      analyticsQueue = batch.concat(analyticsQueue).slice(-ANALYTICS_MAX_QUEUE);
    } finally {
  ...
  function detectEntryPoint() {
    try {
      ...
      if (tgLocal && tgLocal.initData) {
        ...
        if (init.start_param) return 'tg_start_param_' + String(init.start_param);
        return 'telegram';
      }
    } catch (_) {}
  ...
  function initAnalytics(initial) {
    ...
    const entryPoint = (initial && initial.entryPoint) || detectEntryPoint();
    trackEvent('app_open', {
  ...
  function trackEvent(name, payload, options) {
    try {
      ...
      if (options && options.sync && navigator && navigator.sendBeacon) {
        try {
          ...
          console.error('[analytics] sendBeacon error, fallback to queue', e);
        }
      }
  ...
  function trackCheckoutSubmit(order, extra) {
    ...
    trackEvent('checkout_submit', {
      ...
      pickup_location: order.pickupMode ? (order.pickupLocation || '') : '',
